...
(function () {
  ...
  var I18N = {
    en: {
      ...
      "help.eyebrow": "Support", "help.title": "Help & support", "help.subtitle": "Questions about licenses, regulations or your account? We’re here to help.", "help.email.t": "Email us", "help.email.d": "We reply within one business day.", "help.email.cta": "proveramike@gmail.com", "help.chat.t": "Live chat", "help.chat.d": "Chat with our team in real time.", "help.chat.cta": "Start a chat", "help.faqTitle": "Frequently asked questions",
  ...
  function CheckoutPage() {
    ...
    return (
      ...
      '<button class="btn btn-primary btn-lg btn-block" data-pay-now style="margin-top:20px">' + icon("lock") + " " + t("checkout.pay") + " " + money(total) + "</button></div></div></div></section>"
    );
  }
  ...
  function HelpPage() {
    ...
    return (
      ...
      '<div class="help-cards">' + helpCard("mail", "help.email") + helpCard("message", "help.chat") + "</div>" +
  ...
  function CookiesPage() {
    return '<section class="page"><div class="container">' + crumbs([{ href: "#/", label: t("common.home") }, { label: "Cookies Policy" }]) + '<div class="page-head"><h1>Cookies Policy</h1><p style="color:var(--muted);font-size:14px">Last updated: June 2026</p></div><div style="max-width:820px;color:var(--muted);line-height:1.7;font-size:15px"><h3 style="margin-top:24px;margin-bottom:12px;color:var(--ink)">What cookies are</h3><p>Cookies are small text files stored on your device by your browser. They help websites remember information about your visit — like your language preferences, session data, or analytics identifiers to ensure a smooth user experience.</p><h3 style="margin-top:24px;margin-bottom:12px;color:var(--ink)">Cookies we use</h3><p><strong style="color:var(--ink)">Strictly necessary</strong></p><p>These cookies are essential for the website to function properly and cannot be disabled:</p><p style="margin-left:12px"><strong>Session cookie:</strong> Keeps your session active while you navigate our site.<br/><br/><strong>License application session:</strong> Stores your fishing license application data temporarily so you can move between steps without losing information.<br/><br/><strong>CSRF token:</strong> Protects our forms and your data from cross-site request forgery attacks.</p><p><strong style="color:var(--ink)">Analytics</strong></p><p>We use minimal analytics to understand how the website performs and to ensure our service is reliable. These cookies are set only with your consent:</p><p style="margin-left:12px"><strong>Aggregated usage data:</strong> Tracks page views, device types, and the country of origin of our visitors.<br/><br/><strong>Traffic source:</strong> Captures UTM parameters from your arrival URL to help us understand which channels our users come from.</p><p><em>Note: We do not perform cross-site tracking or build individual user profiles.</em></p><p><strong style="color:var(--ink)">Marketing</strong></p><p>If you arrive at our website through an advertising link with UTM parameters, we store those parameters in session storage (not as a cookie) to attribute your order to the correct campaign. This process does not identify you personally.</p><h3 style="margin-top:24px;margin-bottom:12px;color:var(--ink)">Managing cookies</h3><p>You can disable or delete cookies in your browser settings at any time. Please note that disabling strictly necessary cookies will prevent the fishing license application form from working correctly.</p><p>In most browsers, you can manage settings here:</p><p style="margin-left:12px"><strong>Chrome:</strong> Settings → Privacy and security → Cookies and other site data.<br/><br/><strong>Firefox:</strong> Settings → Privacy & Security → Cookies and Site Data.<br/><br/><strong>Safari:</strong> Preferences → Privacy.</p><h3 style="margin-top:24px;margin-bottom:12px;color:var(--ink)">Updates</h3><p>We update this policy when the technology or cookies we use change. The date at the top reflects the latest revision.</p></div></div></section>';
  }

  function ContactPage() {
    return '<section class="page"><div class="container">' + crumbs([{ href: "#/", label: t("common.home") }, { label: "Contact Us" }]) + '<div class="page-head"><h1>Contact Us</h1><p>Get in touch with our team. We\'re here to help with any questions about fishing licenses or your account.</p></div><div class="form-wrap"><form class="form-card" data-form="contact"><h3 style="margin-bottom:18px">Send us a message</h3><div class="field"><label>Name</label><input name="name" required></div><div class="field"><label>Email</label><input type="email" name="email" required></div><div class="field"><label>Subject</label><input name="subject" required></div><div class="field"><label>Message</label><textarea name="message" required style="min-height:150px;resize:vertical;padding:13px 14px;border:1.5px solid var(--line-2);border-radius:10px;font-family:inherit;font-size:15px"></textarea></div><button class="btn btn-primary btn-lg btn-block" type="submit">Send Message</button></form><aside class="form-aside"><h3>Other ways to reach us</h3><ul style="margin-top:20px"><li>' + icon("mail") + ' Email: proveramike@gmail.com</li></ul></aside></div></div></section>';
  }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (el.closest("[data-pay-now]")) { e.preventDefault(); lastOrder = { ref: "ORD-" + Date.now(), total: cartTotal() }; state.cart = []; save(); render(); return; }
